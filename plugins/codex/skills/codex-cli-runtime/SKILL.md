...
description: Internal helper contract for calling the codex-companion runtime from Grok Build
...
- If the forwarded request includes `--background` or `--wait`, treat that as host-side execution control only. Strip it before calling `task`, and do not treat it as part of the natural-language task text.
