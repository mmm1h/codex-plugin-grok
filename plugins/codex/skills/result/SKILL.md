...
- If the raw arguments above are empty or whitespace-only, skip `args-path` and `search_replace`, then run `node "${GROK_PLUGIN_ROOT}/scripts/codex-companion.mjs" result`.
- Otherwise:
  1. Run `node "${GROK_PLUGIN_ROOT}/scripts/codex-companion.mjs" args-path result` and capture its one-line stdout. It is a trusted, absolute path that does not exist yet.
  ...
  3. Run `node "${GROK_PLUGIN_ROOT}/scripts/codex-companion.mjs" result --args-file "<trusted path from step 1>"`.
